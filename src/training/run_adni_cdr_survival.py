#!/usr/bin/env python3
"""
ADNI CDR 恶化生存预测 — Phase 2C-3
=====================================
基于 Phase 2B v2 的统一 LightGBM 特征选择方法论 (s01-s04 LGB + s05 RSF/Cox),
将生存结局从 DXSUM Dementia 替换为 CDR 恶化 (time to CDR worsening).

CDR 恶化定义: 基线 CDR=0→≥0.5, 基线 CDR=0.5→≥1.0
人群: CN + MCI (baseline CDGLOBAL=0 or 0.5, ≥2 CDR visits, surv_time>0)

用法:  python src/training/run_adni_cdr_survival.py
"""

import os, sys, warnings, time
import numpy as np; import pandas as pd
from collections import Counter
from scipy.cluster.hierarchy import linkage, fcluster
from scipy.spatial.distance import squareform
from sklearn.model_selection import StratifiedKFold
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import roc_auc_score, brier_score_loss
from lightgbm import LGBMClassifier
from lifelines import CoxPHFitter, KaplanMeierFitter
from lifelines.utils import concordance_index as lifelines_ci
from sksurv.ensemble import RandomSurvivalForest
import matplotlib; matplotlib.use('Agg'); import matplotlib.pyplot as plt

warnings.filterwarnings('ignore')
t0 = time.time()

# ═══════════════════════════════ CONFIG ═══════════════════════════════
BASE = os.environ.get('PROJECT_ROOT',
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
ADNI_DATA_DIR = os.environ.get('ADNI_DATA_DIR',
    os.path.join(BASE, 'local_data', 'adni'))
DATA_PATH = os.path.join(ADNI_DATA_DIR, 'processed', 'ADNI_baseline_with_time_targets_v2.csv')
RESULTS_DIR = os.path.join(BASE, 'local_data', 'Results_adni', 'cdr_survival')
os.makedirs(RESULTS_DIR, exist_ok=True)

N_SPLITS = 5; SEED = 2022
TOP_S01 = 50; CLUST_TH = 0.75
TOP_SFS = 15; TOP_FINAL = 10; STOP = 0.0005

LGB_PARAMS = dict(
    n_estimators=500, max_depth=15, num_leaves=10,
    subsample=0.7, learning_rate=0.01, colsample_bytree=0.7,
    objective='binary', is_unbalance=True, metric='auc',
    verbosity=-1, seed=2020, n_jobs=4,
    min_data_in_leaf=5, min_gain_to_split=0.0,
)

# Exclusions
COG_PFX = ['MMSE_', 'MOCA_', 'ADAS_', 'FAQ_', 'GDS_', 'NPIQ_', 'NB_', 'HACH_']
CDR_COLS = ['CDMEMORY', 'CDORIENT', 'CDJUDGE', 'CDCOMMUN',
            'CDHOME', 'CDCARE', 'CDGLOBAL', 'CDRSB']
ID_COLS = ['PTID', 'RID', 'PHASE', 'VISCODE', 'VISCODE2',
           'APOE_genotype', 'subject_id', 'entry_research_group', 'PTDOBYY']
TARGET_COLS = [
    'AD_status', 'MCI_status', 'dementia_status', 'DIAGNOSIS',
    'DXNORM', 'DXNODEP', 'DXMCI', 'DXDSEV',
    'DXDDUE', 'DXAD', 'DXPARK', 'DXDEP', 'DXOTHDEM',
    'AD_3yrs', 'AD_5yrs', 'AD_10yrs', 'AD_status_incident', 'AD_years',
    'Dementia_3yrs', 'Dementia_5yrs', 'Dementia_10yrs', 'Dementia_status_incident',
    'Dementia_years', 'converted_to_ad', 'converted_to_dementia', 'converted_to_mci',
    'ad_conversion_years', 'dementia_conversion_years', 'baseline_diagnosis',
    'censored_ad_3yr', 'censored_ad_5yr', 'censored_ad_10yr',
    'censored_dementia_3yr', 'censored_dementia_5yr', 'censored_dementia_10yr',
    'last_followup_years', 'diag_label',
    # CDR targets (exclude from features)
    'CDR_worsen_3yr', 'CDR_worsen_5yr', 'CDR_worsen_10yr', 'CDR_worsen_alltime',
    'CDR_worsen_censored_3yr', 'CDR_worsen_censored_5yr', 'CDR_worsen_censored_10yr',
    'CDR_surv_time', 'CDR_surv_event',
    'CDRSB_surv_time', 'CDRSB_surv_event',
    'CDR_worsened', 'CDR_worsening_years',
    'CDR_worsened_CDRSB2', 'CDRSB_worsening_years',
    'CDR_last_followup_years', 'n_cdr_visits',
    'baseline_CDGLOBAL', 'baseline_CDRSB',
]
IMG_PFX = ('FS_', 'BSI_', 'WMH_', 'AMY_', 'TAU_', 'TAUPVC_')

# ═══════════════════════════════ HELPERS ═══════════════════════════════
def norm_imp(d):
    s = sum(d.values())
    return d if s == 0 else {k: v / s for k, v in d.items()}

def c_index(yt, ye, yp):
    try: return lifelines_ci(yt, -yp, ye)
    except Exception: return 0.5

def make_surv_y(yt, ye):
    return np.array([(bool(e), t) for e, t in zip(ye, yt)],
                     dtype=[('event', bool), ('time', float)])

# ═══════════════════════════════ 1. SURVIVAL LABELS ═══════════════════
print("=" * 70)
print("  Phase 2C-3: CDR Worsening Survival Prediction")
print("  (Unified LGB s01-s04 + RSF/Cox s05)")
print("=" * 70)

df = pd.read_csv(DATA_PATH, low_memory=False)

# Target population: CDR=0 or 0.5, ≥2 CDR visits, non-zero follow-up
at_risk = df[df['baseline_CDGLOBAL'].isin([0.0, 0.5])].copy()
at_risk = at_risk[
    (at_risk['n_cdr_visits'] >= 2) &
    (at_risk['CDR_surv_time'] > 0)
]
cdr_v = at_risk.copy()
cdr_v['surv_time'] = cdr_v['CDR_surv_time']
cdr_v['surv_event'] = cdr_v['CDR_surv_event'].astype(int)

print(f"\n[1] Target population: CN+MCI (CDR 0 or 0.5, ≥2 CDR visits)")
print(f"    Subjects: {len(cdr_v)}")
print(f"    Events: {cdr_v['surv_event'].sum()} "
      f"(median time {cdr_v.loc[cdr_v['surv_event']==1, 'surv_time'].median():.1f}yr)")
print(f"    Censored: {(cdr_v['surv_event']==0).sum()} "
      f"(median f/u {cdr_v.loc[cdr_v['surv_event']==0, 'surv_time'].median():.1f}yr)")

# Breakdown by baseline CDR
for bl_val, label in [(0.0, 'CN (CDR=0)'), (0.5, 'MCI (CDR=0.5)')]:
    sub = cdr_v[cdr_v['baseline_CDGLOBAL'] == bl_val]
    ev = sub['surv_event'].sum()
    print(f"    {label}: n={len(sub)}, events={int(ev)}, censored={int(len(sub)-ev)}")
cdr_v[['RID', 'surv_time', 'surv_event']].to_csv(
    os.path.join(RESULTS_DIR, 'cdr_survival_labels.csv'), index=False)

# ═══════════════════════════════ 2. FEATURES ═══════════════════════════
SURV_COLS = ['surv_time', 'surv_event']
all_ex = set(ID_COLS + TARGET_COLS + CDR_COLS + SURV_COLS)
feat_all = [c for c in cdr_v.columns
            if c not in all_ex
            and not any(c.startswith(p) for p in COG_PFX)]
X_raw = cdr_v[[c for c in feat_all if c in cdr_v.columns]].copy()
for col in X_raw.columns:
    if X_raw[col].isna().any():
        X_raw[col] = X_raw[col].fillna(X_raw[col].median())
const_cols = [c for c in X_raw.columns if X_raw[c].nunique() <= 1]
X = X_raw.drop(columns=const_cols)
yt = cdr_v['surv_time'].values
ye = cdr_v['surv_event'].values.astype(int)
print(f"\n[2] Features: {X.shape[1]} (dropped {len(const_cols)} constant)")

y_binary = ye  # for LightGBM s01-s04
kf = StratifiedKFold(N_SPLITS, random_state=SEED, shuffle=True)

# ═══════════════════════════════ 3. s01: LGB GAIN ═══════════════════════
print(f"\n[3] s01: LightGBM Gain ranking (5-fold CV)...")
tg_cv = Counter()
for tr, te in kf.split(X, y_binary):
    gbm = LGBMClassifier(**LGB_PARAMS)
    gbm.fit(X.iloc[tr], y_binary[tr])
    tg = dict(zip(gbm.booster_.feature_name(),
                  gbm.booster_.feature_importance(importance_type='gain')))
    tg_cv += Counter(norm_imp(tg))
s01_r = sorted(tg_cv.items(), key=lambda x: -x[1])
top50 = [f for f, _ in s01_r[:min(TOP_S01, len(s01_r))]]
n_img = sum(1 for f in top50 if f.startswith(IMG_PFX))
print(f"    Top 50: {n_img} imaging + {len(top50)-n_img} bio, #1={top50[0]}")

# ═══════════════════════════════ 4. s02: WARD ═══════════════════════════
Xt = X[top50].fillna(X[top50].median())
corr = np.array(Xt.corr('spearman'))
corr = np.nan_to_num(corr); corr = (corr + corr.T) / 2
np.fill_diagonal(corr, 1.0)
cl = fcluster(linkage(squareform(1 - np.abs(corr)), 'ward'),
              t=CLUST_TH, criterion='distance')
kept = []; seen = set()
for f, c in zip(top50, cl):
    if c not in seen: kept.append(f); seen.add(c)
Xk = X[kept]
print(f"    s02: {len(top50)} → {len(kept)} features")

# ═══════════════════════════════ 5. s03: LGB RE-RANK ═══════════════════
tg_cv3 = Counter()
for tr, te in kf.split(Xk, y_binary):
    gbm = LGBMClassifier(**LGB_PARAMS)
    gbm.fit(Xk.iloc[tr], y_binary[tr])
    tg = dict(zip(gbm.booster_.feature_name(),
                  gbm.booster_.feature_importance(importance_type='gain')))
    tg_cv3 += Counter(norm_imp(tg))
s03f = [f for f, _ in sorted(tg_cv3.items(), key=lambda x: -x[1])]
print(f"    s03: Top3={s03f[:3]}")

# ═══════════════════════════════ 6. s04: LGB SFS ═══════════════════════
print(f"    s04: SFS (LightGBM AUC)...")
selected = []; remaining = list(s03f); prev_auc = 0; sfs_hist = []
for step in range(min(TOP_SFS, len(s03f))):
    best_f, best_auc = None, -1
    pool = remaining[:min(15, len(remaining))]
    for cand in pool:
        trial = selected + [cand]; aucs = []
        for tr, te in kf.split(Xk[trial], y_binary):
            g = LGBMClassifier(**LGB_PARAMS)
            g.fit(Xk[trial].iloc[tr], y_binary[tr])
            aucs.append(roc_auc_score(y_binary[te],
                         g.predict_proba(Xk[trial].iloc[te])[:, 1]))
        avg_auc = np.mean(aucs)
        if avg_auc > best_auc: best_f, best_auc = cand, avg_auc
    if best_f is None: break
    gain = best_auc - prev_auc if step > 0 else best_auc
    isi = best_f.startswith(IMG_PFX)
    sfs_hist.append({'step': step + 1, 'feature': best_f,
                     'auc': best_auc, 'gain': gain, 'is_imaging': isi})
    print(f"      Step {step+1}: {'[IMG]' if isi else '[BIO]'} {best_f}  "
          f"AUC={best_auc:.4f} (Δ={gain:+.4f})")
    if gain < STOP and len(selected) >= TOP_FINAL:
        selected.append(best_f); remaining.remove(best_f); break
    selected.append(best_f); remaining.remove(best_f); prev_auc = best_auc

top_f = selected[:TOP_FINAL]
n_img_f = sum(1 for f in top_f if f.startswith(IMG_PFX))
print(f"    Top {len(top_f)}: {n_img_f} imaging + {len(top_f)-n_img_f} bio")
pd.DataFrame(sfs_hist).to_csv(os.path.join(RESULTS_DIR, 'sfs_history.csv'), index=False)

# ═══════════════════════════════ 7. s05: RSF + Cox ═══════════════════
print(f"\n[7] s05: RSF (main) + Cox PH (baseline)...")
Xf = Xk[top_f]
sc2 = StandardScaler(); Xf_s = pd.DataFrame(
    sc2.fit_transform(Xf), columns=Xf.columns, index=Xf.index)
sy_full = make_surv_y(yt, ye)

# RSF
rsf_cis = []; tp_buf = {1:[],3:[],5:[]}; tb_buf = {1:[],3:[],5:[]}
for tr, te in kf.split(Xf_s, ye):
    rsf = RandomSurvivalForest(n_estimators=200, max_depth=5, min_samples_leaf=5,
                               random_state=SEED, n_jobs=4)
    rsf.fit(Xf_s.iloc[tr].values, make_surv_y(yt[tr], ye[tr]))
    pr = rsf.predict(Xf_s.iloc[te].values)
    rsf_cis.append(c_index(yt[te], ye[te], pr))
    for t_eval in [1, 3, 5]:
        yb_t = ((yt[te] <= t_eval) & (ye[te] == 1)).astype(int)
        valid_t = ((ye[te] == 1) & (yt[te] <= t_eval)) | (yt[te] > t_eval)
        tp_buf[t_eval].extend(pr[valid_t]); tb_buf[t_eval].extend(yb_t[valid_t])

rsf_cidx = np.mean(rsf_cis); rsf_std = np.std(rsf_cis)
print(f"    RSF: C-index = {rsf_cidx:.4f} ± {rsf_std:.4f}")
taucs, tbriers = {}, {}
for t_eval in [1, 3, 5]:
    if len(set(tb_buf[t_eval])) >= 2 and sum(tb_buf[t_eval]) >= 2:
        taucs[t_eval] = roc_auc_score(tb_buf[t_eval], tp_buf[t_eval])
        pr_arr = np.array(tp_buf[t_eval])
        pn = (pr_arr - pr_arr.min()) / (pr_arr.max() - pr_arr.min() + 1e-8)
        tbriers[t_eval] = brier_score_loss(tb_buf[t_eval], pn)
        print(f"    tAUC@{t_eval}yr = {taucs[t_eval]:.4f}, Brier = {tbriers[t_eval]:.4f}, "
              f"n = {len(tb_buf[t_eval])}")

# Cox PH
try:
    cph_df = Xf_s.copy(); cph_df['time'] = yt; cph_df['event'] = ye
    lv = [c for c in Xf_s.columns if Xf_s[c].std() < 0.001]
    if lv: cph_df = cph_df.drop(columns=lv)
    cph = CoxPHFitter(penalizer=0.1); cph.fit(cph_df, 'time', 'event')
    cox_ci_val = cph.concordance_index_
    print(f"    Cox PH: C-index = {cox_ci_val:.4f}")
    cph.summary[['coef', 'exp(coef)', 'p']].sort_values('p').to_csv(
        os.path.join(RESULTS_DIR, 'cox_ph_summary.csv'))
except Exception as e:
    print(f"    Cox PH failed: {e}"); cox_ci_val = np.nan

# ═══════════════════════════════ 8. SUMMARY ═══════════════════════════
print(f"\n{'=' * 70}")
print(f"  CDR SURVIVAL RESULTS")
print(f"{'=' * 70}")
print(f"\n  {'Model':<22} {'C-index':<12} {'tAUC@1yr':<10} "
      f"{'tAUC@3yr':<10} {'tAUC@5yr':<10}")
print(f"  {'─' * 22} {'─' * 12} {'─' * 10} {'─' * 10} {'─' * 10}")
for name, ci, a1, a3, a5 in [
    ('RSF CDR',       rsf_cidx, taucs.get(1, np.nan), taucs.get(3, np.nan), taucs.get(5, np.nan)),
    ('Cox PH CDR',    cox_ci_val, np.nan, np.nan, np.nan),
    ('RSF DXSUM (P2B)', 0.7648, np.nan, 0.8032, 0.8342),
]:
    ci_s = f"{ci:.4f}" if not np.isnan(ci) else "—"
    a1_s = f"{a1:.4f}" if not np.isnan(a1) else "—"
    a3_s = f"{a3:.4f}" if not np.isnan(a3) else "—"
    a5_s = f"{a5:.4f}" if not np.isnan(a5) else "—"
    print(f"  {name:<22} {ci_s:<12} {a1_s:<10} {a3_s:<10} {a5_s:<10}")

if not np.isnan(taucs.get(3, np.nan)):
    print(f"\n  RSF CDR vs RSF DXSUM @3yr: ΔtAUC = {taucs[3]-0.8032:+.4f}")
if not np.isnan(taucs.get(5, np.nan)):
    print(f"  RSF CDR vs RSF DXSUM @5yr: ΔtAUC = {taucs[5]-0.8342:+.4f}")

print(f"\n  Top 10 CDR survival features:")
for i, f in enumerate(top_f[:10]):
    s01_rk = next((j+1 for j, (fn, _) in enumerate(s01_r) if fn == f), '—')
    tag = '[IMG]' if f.startswith(IMG_PFX) else '[BIO]'
    print(f"    {i+1:>2}. {tag} {f:<30} (s01 #{s01_rk})")

# Save
pd.DataFrame({
    'model': ['RSF CDR', 'Cox PH CDR', 'RSF DXSUM', 'Binary CDR 3yr', 'Binary CDR 5yr'],
    'c_index': [rsf_cidx, cox_ci_val, 0.7648, np.nan, np.nan],
    'auc_1yr': [taucs.get(1, np.nan), np.nan, np.nan, np.nan, np.nan],
    'auc_3yr': [taucs.get(3, np.nan), np.nan, 0.8032, 0.8079, np.nan],
    'auc_5yr': [taucs.get(5, np.nan), np.nan, 0.8342, np.nan, 0.8282],
    'brier_3yr': [tbriers.get(3, np.nan), np.nan, np.nan, 0.1332, np.nan],
    'brier_5yr': [tbriers.get(5, np.nan), np.nan, np.nan, np.nan, 0.1627],
}).to_csv(os.path.join(RESULTS_DIR, 'model_comparison_cdr.csv'), index=False)
pd.DataFrame({'rank': range(1, len(top_f) + 1), 'feature': top_f}).to_csv(
    os.path.join(RESULTS_DIR, 'selected_features_cdr.csv'), index=False)
pd.DataFrame([{'s01_rank': i+1, 'feature': f, 's01_gain': g}
              for i, (f, g) in enumerate(s01_r)]).to_csv(
    os.path.join(RESULTS_DIR, 'lgb_feature_ranking_cdr.csv'), index=False)

# ═══════════════════════════════ 9. PLOTS ═══════════════════════════════
print(f"\n[9] Plots...")

# 9a. KM curves
rsf_all = RandomSurvivalForest(n_estimators=300, max_depth=5, min_samples_leaf=5,
                               random_state=SEED, n_jobs=4)
rsf_all.fit(Xf_s.values, sy_full)
all_risk = rsf_all.predict(Xf_s.values)
risk_g = pd.qcut(all_risk, 3, labels=['Low Risk', 'Medium Risk', 'High Risk'])
fig, ax = plt.subplots(figsize=(10, 6))
for lb, color in [('Low Risk', '#10B981'), ('Medium Risk', '#F59E0B'),
                   ('High Risk', '#EF4444')]:
    mask = risk_g == lb
    kmf = KaplanMeierFitter()
    kmf.fit(yt[mask], ye[mask], label=f'{lb} (n={mask.sum()})')
    kmf.plot_survival_function(ax=ax, color=color, linewidth=2.5)
ax.set_xlabel('Years from Baseline', fontsize=13)
ax.set_ylabel('CDR Worsening-free Probability', fontsize=13)
ax.set_title('CDR Worsening: KM Curves by RSF Risk Tertile',
             fontweight='bold', fontsize=14)
ax.set_xlim(0, 8); ax.set_ylim(0, 1.05)
ax.legend(fontsize=11, loc='lower left'); ax.grid(alpha=0.25)
fig.tight_layout()
fig.savefig(os.path.join(RESULTS_DIR, 'km_by_risk_cdr.png'), dpi=150)
plt.close()

# 9b. SFS curve
sd = pd.DataFrame(sfs_hist)
fig2, ax2 = plt.subplots(figsize=(10, 6))
ax2.plot(sd['step'], sd['auc'], 'o-', color='#8B5CF6', lw=2.5, ms=10, mfc='white', mew=2)
for _, r in sd.iterrows():
    ax2.text(r['step'], r['auc'] + 0.006, r['feature'][:20], fontsize=7, ha='center')
ax2.set_xlabel('SFS Step', fontsize=13)
ax2.set_ylabel('AUC (LightGBM CV)', fontsize=13)
ax2.set_title('SFS Feature Accumulation — CDR Worsening (LGB AUC)',
              fontweight='bold', fontsize=14)
ax2.grid(alpha=0.25)
fig2.tight_layout()
fig2.savefig(os.path.join(RESULTS_DIR, 'sfs_accumulation_cdr.png'), dpi=150)
plt.close()

# 9c. Calibration
fig3, axes = plt.subplots(1, 3, figsize=(16, 5))
for ai, tev in enumerate([1, 3, 5]):
    ax = axes[ai]; ar, ab = [], []
    for tr, te in kf.split(Xf_s, ye):
        rsf_f = RandomSurvivalForest(n_estimators=200, max_depth=5, min_samples_leaf=5,
                                      random_state=SEED, n_jobs=4)
        rsf_f.fit(Xf_s.iloc[tr].values, make_surv_y(yt[tr], ye[tr]))
        pr = rsf_f.predict(Xf_s.iloc[te].values)
        pn = (pr - pr.min()) / (pr.max() - pr.min() + 1e-8)
        yb = ((yt[te] <= tev) & (ye[te] == 1)).astype(int)
        valid = ((ye[te] == 1) & (yt[te] <= tev)) | (yt[te] > tev)
        ar.extend(pn[valid]); ab.extend(yb[valid])
    if len(set(ab)) >= 2:
        qs = np.percentile(ar, np.linspace(0, 100, 11)); bc, bo = [], []
        for i in range(len(qs) - 1):
            mk = (np.array(ar) >= qs[i]) & (np.array(ar) < qs[i + 1])
            if mk.sum() >= 5: bc.append(np.mean(np.array(ar)[mk])); bo.append(np.mean(np.array(ab)[mk]))
        if bc: ax.plot(bc, bo, 'o-', color='#8B5CF6', lw=2.5, ms=10, mfc='white', mew=2)
        ax.plot([0, 1], [0, 1], '--', color='gray', alpha=0.4)
        ax.set_xlabel('Predicted'); ax.set_ylabel('Observed')
        ax.set_title(f'{tev}-Year Calibration', fontweight='bold')
        ax.grid(alpha=0.2)
fig3.suptitle('RSF Calibration: CDR Worsening Risk', fontweight='bold', fontsize=14)
fig3.tight_layout()
fig3.savefig(os.path.join(RESULTS_DIR, 'calibration_cdr.png'), dpi=150)
plt.close()

# 9d. Feature ablation
ablation_results = []
for i in range(len(top_f), 0, -1):
    sub_f = top_f[:i]; cis_i = []
    for tr, te in kf.split(Xf_s[sub_f], ye):
        rsf_a = RandomSurvivalForest(n_estimators=100, max_depth=5, min_samples_leaf=5,
                                      random_state=SEED, n_jobs=4)
        rsf_a.fit(Xf_s[sub_f].iloc[tr].values, make_surv_y(yt[tr], ye[tr]))
        pr_a = rsf_a.predict(Xf_s[sub_f].iloc[te].values)
        cis_i.append(c_index(yt[te], ye[te], pr_a))
    ablation_results.append({'n_features': i, 'c_index': np.mean(cis_i),
                             'c_index_std': np.std(cis_i)})
pd.DataFrame(ablation_results).to_csv(
    os.path.join(RESULTS_DIR, 'feature_ablation.csv'), index=False)

elapsed = time.time() - t0
print(f"\n{'=' * 70}")
print(f"  ✅ CDR Survival completed in {elapsed / 60:.1f} min")
print(f"  📁 Results: {RESULTS_DIR}")
for f in sorted(os.listdir(RESULTS_DIR)):
    p = os.path.join(RESULTS_DIR, f)
    if os.path.isfile(p): print(f"     {f} ({os.path.getsize(p)/1024:.0f} KB)")
print(f"{'=' * 70}")
