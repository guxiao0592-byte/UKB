...
# ==============================================================================
# SLIDE 8: IMAGING SUBSET ANALYSIS — WHY FULL-COHORT IS RIGHT
# ==============================================================================
s = add_blank_slide()
add_title_bar(s, "影像子集分析: 为什么不能用仅 MRI 人群独立训练", "影像子集的严重健康志愿者偏差使独立训练不可行")

add_textbox(s, 0.8, 1.7, 11.5, 0.4, "队列对比: 全队列 vs 影像子集", font_size=16, color=BLUE, bold=True)
add_simple_table(s, 0.8, 2.1, [2.5, 2.5, 2.5, 2.5, 2.5],
    ["指标", "全队列 (Deploy)", "影像子集 (独立SFS)", "差异", "影响"],
    [
        ["参与者", "494,658", "46,104", "-91%", "样本量骤降"],
        ["DM_full 阳性", "9,545 (1.93%)", "148 (0.32%)", "-98%", "每折仅30例，方差极大"],
        ["AD_full 阳性", "4,315 (0.87%)", "62 (0.13%)", "-99%", "每折12例，无法训练"],
        ["AUC (s04基线)", "0.8373 (10特征)", "0.8948 (10特征)", "虚高", "s04过拟合，s05崩回0.88"],
        ["AUC (s05校准)", "0.837 ± 0.004", "0.883 ± 0.027", "+0.046但σ×7", "方差大7倍，不可靠"],
        ["Top50影像占比", "6/50 (12%)", "45/50 (90%)", "完全反转", "临床特征被影像噪声淹没"],
        ["特征选择", "1 影像 + 9 临床", "10 全影像", "临床消失", "过拟合+样本选择偏差"],
    ], font_size=9)

add_multiline(s, 0.8, 5.2, 12.0, 2.0, [
    ("核心发现: 影像子集 = 健康志愿者偏差", True, 14, BLUE),
    ("", False, 4, DARK),
    ("• 脑 MRI 需要参与者亲自到影像中心 → 已患痴呆者几乎不会参加 → 阳性率从 1.93% 暴跌至 0.32% (差6倍)", False, 12, DARK),
    ("• 仅 148 例痴呆，5折 CV 每折仅 ~30 例 → s05 AUC 跨折波动 0.83-0.91 (±0.027 vs 全队列 ±0.004)", False, 12, DARK),
    ("• 小样本下脑结构特征(高维、共线性强)过拟合严重 → s04 选中10/10影像特征但 s05 性能崩回", False, 12, DARK),
    ("• 全队列 + LightGBM NaN 处理不是 workaround，而是利用全部 9,545 例样本的唯一正确方法论选择", False, 12, GREEN),
    ("• 该发现对 UK Biobank 影像学研究具有通用启示: 影像子集的选择偏差必须在研究设计阶段考虑", False, 12, GRAY),
])
add_page_number(s, 8, TOTAL_SLIDES)

# ==============================================================================
# SLIDE 9: BEFORE/AFTER COMPARISON CHART
# ==============================================================================
s = add_blank_slide()
